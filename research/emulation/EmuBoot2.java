...
public class EmuBoot2 extends GhidraScript {
  EmulatorHelper emu;
  ...
  long loadEA(long pc){
    ...
    for(int o=0;o<in.getNumOperands();o++){
      Object[] objs=in.getOpObjects(o); Register base=null; long disp=0; boolean hasScalar=false; long absAddr=-1;
      for(Object x:objs){
        if(x instanceof Register) base=(Register)x;
        else if(x instanceof Scalar){ disp=((Scalar)x).getSignedValue(); hasScalar=true; }
        else if(x instanceof Address) absAddr=((Address)x).getOffset();   // absolute load operand (ld.bu d2,0xd000177a)
      }
      if(base!=null){ try{ return emu.readRegister(base).longValue()+disp; }catch(Exception e){} }
      if(hasScalar && disp!=0) return disp;
      if(absAddr>=0) return absAddr;          // <-- the fix: Address operands, not just Scalars
    }
    return -1;
  }
  long refReadEA(long pc){
    Instruction in=getInstructionAt(A(pc)); if(in==null) return -1;
    String mn=in.getMnemonicString().toLowerCase(); if(!mn.startsWith("ld")) return -1;
    for(Reference r: in.getReferencesFrom()){ RefType t=r.getReferenceType();
      if(t!=null && (t.isRead()||t.isData())){ long a=r.getToAddress().getOffset(); if(a>0) return a; } }
    return -1;
  }
  ...
  public void run() throws Exception {
    ...
    try{ org.apache.logging.log4j.core.config.Configurator.setLevel("ghidra.pcode.emulate.Emulate", org.apache.logging.log4j.Level.OFF);
         ...
         org.apache.logging.log4j.core.config.Configurator.setLevel("ghidra.app.emulator", org.apache.logging.log4j.Level.OFF);
         org.apache.logging.log4j.core.config.Configurator.setLevel("ghidra.app.emulator.EmulatorHelper", org.apache.logging.log4j.Level.OFF);
         org.apache.logging.log4j.core.config.Configurator.setRootLevel(org.apache.logging.log4j.Level.FATAL); }catch(Throwable t){}
    for(long[] r: new long[][]{{0x0L,0x1000},{0xc0000000L,0x400000},{0xd0000000L,0x20000},
    ...
    // KNOWN completion-flag pre-sets (async-set bytes the coerced boot never sets): 0xd000177a = subsystem-ready
    long[] preFlags = {0xd000177aL};
    for(long f: preFlags){ try{ emu.writeMemory(A(f), new byte[]{1}); }catch(Exception e){} }
    java.util.Set<Long> discoveredFlags=new java.util.LinkedHashSet<>();
    ...
    java.util.HashMap<Long,Integer> brHits=new java.util.HashMap<>(); int pollBreaks=0; long lastBrClear=0; int faultLog=0; long lastHb=0;
    ...
    Set<Long> retStub=new HashSet<>(Arrays.asList(0x801d7838L,0x8002ed62L,0x8002ed60L,0x801d7902L));
    ...
    try{
      for(;steps<budget;steps++){
        ...
        if(steps-lastHb>=1000000){ lastHb=steps; println(String.format("  .. hb step%d pc=%x callTargets=%d ctx=%x",steps,pc,callTargets.size(),rU32(0xc03fc37cL))); }
        ...
        if(steps%15000==0){ w32(0xf0000240L, 0x2000L);
          for(long f: discoveredFlags){ try{ w32(f&~3L,0xffffffffL);}catch(Exception e){} }
          try{ emu.writeMemory(A(0xd000177aL),new byte[]{1}); }catch(Exception e){} }
        // per-step ctx watch
        ...
        // DECEL-CAL read-watch: catch ANY load into the decel-limit curve 0x8004dd90-0x8004ddbe
        // (axis@dd90, functional y@dda0, monitor y@ddb0) and its non-cached alias 0xa004dd9x.
        // NOT gated on `injected` -> watches from step 0 so the L2 symmetry/integrity monitor
        // (which runs at init / cyclically, not ACC-gated) is caught. Dedup by (pc,ea) so a
        // loop reading multiple cells shows each block, and a fn reading BOTH dda0 & ddb0 (the
        // symmetry monitor) is visible as two hits from nearby PCs.
        if(readWatch && mn.startsWith("ld")){ long ea=loadEA(pc);
          if(ea>=0 && ((ea>=0x8004dd90L&&ea<0x8004ddc0L)||(ea>=0xa004dd90L&&ea<0xa004ddc0L))
             && rwSeen.add((pc<<20)^(ea&0xfffffL))){
            ghidra.program.model.listing.Function fn=getFunctionContaining(A(pc));
            println(String.format("  RD-DECEL %x reads [%x] %s (fn %s) @step%d",pc,ea,in.toString(),fn!=null?fn.getName():"?",steps)); } }
        ...
        ring.addLast(pc); if(ring.size()>6000){ ring.pollFirst();
          if((steps&0xfff)==0){ HashSet<Long> s=new HashSet<>(ring);
            if(s.size()<=120){                 // stuck in a small PC region (single- or multi-insn poll loop)
              ...
              boolean poked=false;
              for(long ppc:s){ long ea=loadEA(ppc); if(ea<0) ea=refReadEA(ppc); if(ea>=0 && pokeable(ea)){ w32(ea&~3L,0xffffffffL); poked=true;
                  if(ea>=0xd0000000L && ea<0xd0010000L && discoveredFlags.add(ea))
                    println(String.format("  >>> DISCOVERED completion-flag poll @%x reads [%x] -> set nonzero (flags now %d) step%d",region,ea,discoveredFlags.size(),steps)); } }
              ...
              ring.clear(); } } }
        prevPc=pc;
        try{ emu.step(monitor); }catch(Throwable se){ if(faultLog<8){faultLog++; println(String.format("  [STEP-THROW @%x : %s]",pc,se.getClass().getSimpleName())); } }
        long np=emu.readRegister("pc").longValue();
        // GENERIC POLL-BREAKER: a conditional branch taken backward (or to self) a huge number of
        // times in a row is a busy-wait on a ready-flag an ISR would set. Force it to fall through,
        // independent of the ready-check function size (the small-loop spin-breaker misses big ones).
        { String bm = in!=null?in.getMnemonicString().toLowerCase():"";
          boolean cond = (bm.startsWith("j")&&!bm.equals("j")&&!bm.equals("ji")&&!bm.equals("jl")&&!bm.equals("jla"))||bm.startsWith("loop");
          int ilen = in!=null?in.getLength():2;
          boolean taken = (np!=pc+ilen);
          if(steps-lastBrClear>300000){ brHits.clear(); lastBrClear=steps; }   // decay: finished loops drop out
          if(cond && taken && np<=pc){
            int h=brHits.merge(pc,1,Integer::sum);
            if(h>40000){   // this backward branch has been taken ~forever -> a ready-flag poll
              emu.writeRegister("pc", pc+ilen);   // force fall-through past the poll
              pollBreaks++; if(pollBreaks<=80) println(String.format("  >>> POLL-BREAK forced fall-through @%x (%s, %d hits) -> %x step%d",pc,bm,h,pc+ilen,steps));
              brHits.remove(pc); ring.clear(); prevPc=pc; continue; }
          }
        }
      ...
      long pc=rreg("pc");
      stop="FAULT("+e.getClass().getSimpleName()+") @pc="+Long.toHexString(pc)+" : "+e.getMessage();
    }
