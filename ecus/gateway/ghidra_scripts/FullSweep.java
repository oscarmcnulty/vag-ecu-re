...
import java.util.*;
// Multi-arch linear-sweep scorer for arch identification (mirrors ecus/eps FullSweep.java).
// args: <start> <len> <imglo> <imghi> [thumb]
//   thumb -> if a "TMode" context register exists, set it to 1 before each disassemble (ARM Thumb).
public class FullSweep extends GhidraScript {
  public void run() throws Exception {
    ...
    boolean thumb=a.length>4 && a[4].equalsIgnoreCase("thumb");
    Register tmode = thumb? currentProgram.getLanguage().getRegister("TMode") : null;
    ...
    while(ad.compareTo(end)<0){
      ...
      try{
        if(tmode!=null){ currentProgram.getProgramContext().setValue(tmode, ad, ad, java.math.BigInteger.ONE); }
        disassemble(ad); ins=getInstructionAt(ad);
      }catch(Exception e){}
